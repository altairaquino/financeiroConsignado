...
function imprimirDiv(id){
	...
	teste = document.getElementById(id).innerHTML;
	 
	var tmp = window.open(' ', 'popimpr');
	// escreve ele
	tmp.document.write('<link rel="stylesheet" href="stylesheet/style.css" type="text/css" media="screen, projection">');
	tmp.document.write(teste);
	// manda imprimir
	tmp.window.print();
	//fecha a janela
	tmp.window.close(); 
...
// use onkeyup="limitaCaractere(this, 10)"
	function limitaCaractere(elem, tamanho){
		elem.value = elem.value.slice(0, tamanho);		
	}
