...
public class FormataObj {
	...
	/***
	 * new com.grupoexata.bancario.utils.FormataObj()
	 */
	...
	private static final String FORMAT_MOEDA_BR = "R$ ##,##0.00;(R$ -##,##0.00)";
	private static final String FORMAT_NUMERO_BR = "##,##0.00;(##,##0.00)";
	...
	public String formataValor(float valor, String format){
		String ret = null;
		try {
			formataNumero = new DecimalFormat(format, formatSymbols);
			ret = formataNumero.format(valor);
		} catch (Exception e) {
			e.printStackTrace();
		}		
	...
	public String formataData(Date data, String formato){
		String ret = null;
		if (data != null){
			formataData = new SimpleDateFormat(formato,local);
			ret = formataData.format(data);
		}
		return ret;
	}
