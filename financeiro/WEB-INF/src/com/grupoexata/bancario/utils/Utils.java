...
public class Utils {
	...
	public static final long DAY_IN_MILISEGUNDOS = 86400000;
	...
	public static java.util.Date incrementaData(java.util.Date data, int dias){
		return new java.util.Date(data.getTime() + (DAY_IN_MILISEGUNDOS * dias));
	}
	
	public static java.util.Date proximaDataPagamento(){	
		return proximaDataPagamento(new java.util.Date());
	}
	
	@SuppressWarnings("deprecation")
	public static java.util.Date proximaDataPagamento(java.util.Date d){
		int dia = Calendar.DAY_OF_WEEK;
		if (dia >= 3 || dia == 0){
			//quarta
			d.setTime(d.getTime() + DAY_IN_MILISEGUNDOS);
			while (d.getDay() != 3){
				d.setTime(d.getTime() + DAY_IN_MILISEGUNDOS);
			}
		}else{
	 ...
	 * @param str String passada para modificação.
	...
	public static java.util.Date stringToTime(String datastr){
	...
	public static String objectToStrBR(String format,Object value){
		return value == null ? null : new DecimalFormat(format,formatSymbols).format(value);
	}
	
	private static final String COUNTRY = "BR";
	private static final String LANGUAGE = "pt";
	
	private static Locale local = new Locale(LANGUAGE,COUNTRY);
	private static DecimalFormatSymbols formatSymbols = new DecimalFormatSymbols(local);
	...
	public static <T> List<T> getObjectsStrFk(Class<T> class0 ,Connection conn, String query, int fk){
		List<T> list = new ArrayList<T>();
		PreparedStatement st = null;
		try {
			st = conn.prepareStatement(query);
			st.setInt(1, fk);
			list = Utils.getObjectsStr(st, class0);
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			try{
	...
	public static <B>B getObjectStrById(Class<B> class0 ,Connection conn, String query, int id) {
		B b = null;
		PreparedStatement st = null;
		try {
			st = conn.prepareStatement(query);
			st.setInt(1, id);
			b = getObjectsStrFirst(st, class0);
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			try{
	...
	public static <T> List<T> getObjectsStr(Class<T> class0 ,Connection conn, String query){
		List<T> list = new ArrayList<T>();
		PreparedStatement st = null;
		try {
			st = conn.prepareStatement(query);
	...
	public static <T> T getObjectsStrFirst(PreparedStatement st, Class<T> class0) {
		T object = null;
		ResultSet rs = null;
		try {
			ResultSetMetaData metaData = st.getMetaData();
			...
			if (rs.next()) {
				...
				for (int i = 1; i <= colls; i++) {
					String className = metaData.getColumnClassName(i);
					String atributo = metaData.getColumnName(i);
					setAtributoStr(object, class0, Class.forName(className),
							rs, atributo);
				}
			}
		} catch (Exception e) {
	...
	public static <T,U> List<T> getObjsStrRec(PreparedStatement st, Class<T> class0){
		List<T> lista = new ArrayList<T>();
		try {
			ResultSetMetaData metaData = st.getMetaData();
			ResultSet rs = st.executeQuery();
			...
			while(rs.next()){
				...
				for (int i = 1; i <= colls; i++) {
					...
					if(atributos.length > 1){
						...
						Object obj0 = object;
						Object obj1 = null;
						for (int j = 0; j < atributos.length -1; j++) {
							Method m = c0.getMethod("get" + toCapitalize(atributos[j]));
							obj1 = m.invoke(obj0);
							c1 = m.getReturnType();
							if(obj1 == null){
								obj1 = c1.newInstance();
								m = c0.getMethod("set" + toCapitalize(atributos[j]) , c1);
								m.invoke(obj0, obj1);
							}
							c0 = c1;
							obj0 = obj1;
						}
						setAtributoStr(c0.cast(obj1), c0, Class.forName(className), rs, atributos[atributos.length -1]);
					}else{
						setAtributoStr(object, class0, Class.forName(className), rs, atributo);
					}
				}
				lista.add(object);
			}
